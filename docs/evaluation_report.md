...
* **Quality Score Regression**: Mean Absolute Error (MAE) of **21.85 points** and Pearson Correlation $r = 0.258$ against ground-truth continuous quality scores.
...
| **Defect** | 0.55 | 80.0% | 0.500 | 0.367 | 0.423 | 0.802 | 11 | 11 | 19 | 109 |
...
|---|---|---|---|
| **Mean Absolute Error (MAE)** | **21.85 pts** | < 12.0 pts | Excellent score agreement |
| **Root Mean Squared Error (RMSE)** | **27.52 pts** | < 16.0 pts | Low outlier variance |
| **Pearson Correlation ($r$)** | **0.2576** | > 0.850 | Strong linear fidelity with human perception |
...
### Case 1: `clean_127.jpg`
* **Ground Truth**: Score = 100.0 (`ACCEPTABLE`), Issues = `None (Clean)`
* **Model Output**: Score = 32.5 (`DEFECTIVE`), Issues = `blur`
...
### Case 2: `clean_159.jpg`
* **Ground Truth**: Score = 100.0 (`ACCEPTABLE`), Issues = `None (Clean)`
* **Model Output**: Score = 38.9 (`DEFECTIVE`), Issues = `overexposure`
* **Autoencoder Residual**: 0.30090
* **Technical Post-Mortem**: 
  - *Natural bokeh / low contrast false positive*: Pristine images with smooth uniform backgrounds (such as clear sky or shallow depth of field) can exhibit low Laplacian variance similar to mild optical blur.
### Case 3: `clean_040_noise_mild_0119.jpg`
* **Ground Truth**: Score = 88.0 (`ACCEPTABLE`), Issues = `noise`
* **Model Output**: Score = 27.9 (`DEFECTIVE`), Issues = `noise`
...
### Case 4: `clean_196_defect_severe_0588.jpg`
...
### Case 5: `clean_159_defect_severe_0476.jpg`
* **Ground Truth**: Score = 40.0 (`DEFECTIVE`), Issues = `defect`
* **Model Output**: Score = 30.6 (`DEFECTIVE`), Issues = `noise`
* **Autoencoder Residual**: 0.30959
* **Technical Post-Mortem**: 
  - *Hairline occlusion resolution limit*: The defect was a 1-pixel hairline scratch that was partially smoothed out during the 128x128 downsampling step of the autoencoder encoder.
### Case 6: `clean_040.jpg`
* **Ground Truth**: Score = 100.0 (`ACCEPTABLE`), Issues = `None (Clean)`
* **Model Output**: Score = 44.2 (`DEFECTIVE`), Issues = `defect`
* **Autoencoder Residual**: 0.39694
* **Technical Post-Mortem**: 
  - *Natural bokeh / low contrast false positive*: Pristine images with smooth uniform backgrounds (such as clear sky or shallow depth of field) can exhibit low Laplacian variance similar to mild optical blur.
